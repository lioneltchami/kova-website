...
export function Footer() {
	return (
		<footer className="border-t border-kova-border bg-kova-surface py-8 px-4">
			<div className="mx-auto max-w-6xl">
				...
				<div className="flex flex-col sm:flex-row items-start justify-between gap-8">
					...
					<div className="flex items-center gap-3 shrink-0">
						...
						<div>
							...
							<p className="text-xs text-kova-silver-dim/70">
					...
					<div className="flex flex-wrap gap-x-12 gap-y-6">
						{Object.entries(links).map(([group, items]) => (
							<div key={group}>
								<h3 className="mb-2 text-xs font-semibold uppercase tracking-widest text-kova-silver-dim/70">
									{group}
								</h3>
								<ul className="space-y-1">
									{items.map((item) => (
										<li key={item.label}>
											<a
												href={item.href}
												className="text-xs text-kova-silver-dim transition-colors hover:text-white"
											>
												{item.label}
											</a>
										</li>
				...
				{/* Bottom line */}
				<div className="mt-6 pt-4 border-t border-kova-border/50 flex flex-col sm:flex-row items-center justify-between gap-2">
					<p className="text-xs text-kova-silver-dim/40">
