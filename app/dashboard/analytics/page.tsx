...
export default async function AnalyticsPage({
  ...
  // --- Anomaly detection on daily cost series ---
  const dailyCostSeries = dailyData.map((d) => ({
    date: d.date,
    cost: d.cost,
  }));
  const anomalyResults = detectAnomalies(dailyCostSeries);

  // --- ROI metrics ---
  const activeDays = dailyData.filter((d) => d.cost > 0).length;
    ...
    <div>
      ...
      {totalCount === 0 ? (
        ...
        <div className="space-y-6">
          ...
          {/* Anomaly detection + ROI metrics side by side */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Anomaly chart */}
            <div className="bg-kova-surface border border-kova-border rounded-xl p-6">
              <h2 className="text-base font-semibold text-white mb-1">
              ...
              <p className="text-xs text-kova-silver-dim mb-4">
                Days with unusual spend highlighted in amber / red
              </p>
              <AnomalyChart data={anomalyResults} />
            </div>
