...
// CSV escaping with injection prevention (M-5).
// Wraps values containing commas, quotes, or newlines in double-quotes and doubles
// any literal quote characters. Prefixes formula-injection triggers (=, +, -, @)
// with a tab so spreadsheet applications do not evaluate them as formulas.
function csvEscape(value: string | null | undefined): string {
	let str = value ?? "";
	// Neutralise CSV formula injection: prefix cells that start with a formula trigger.
	if (/^[=+\-@]/.test(str)) {
		str = `\t${str}`;
	}
	if (/[",\n\r]/.test(str)) {
